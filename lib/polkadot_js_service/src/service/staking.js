...
async function _loadSome(api, fromHash, toHash) {
...
function _extractRewardData(validatorId, blockCounts, sessionRewards) {
  ...
  sessionRewards.forEach(
    ({ blockNumber, reward, sessionIndex, slashes }, index) => {
      ...
      blocksLabels.push(formatNumber(sessionIndex));
  ...
  const { idxSet, avgSet } = _extractblockData(blockCounts);
  return { rewardsChart, rewardsLabels, idxSet, avgSet, blocksLabels };
}

function _extractblockData(blockCounts) {
  const avgSet = [];
  const idxSet = [];
  if (blockCounts) {
    blockCounts.reduce((total, value, index) => {
      total = total.addn(value);
...
async function loadValidatorStakeData(api, validatorId) {
  ...
  const splitMax = splitChart
    ...
    : 100;
  return { stakeChart, stakeLabels, splitChart, splitMax };
}
